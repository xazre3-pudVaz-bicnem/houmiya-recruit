          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="animate-pulse">
                <div className="bg-gray-100 aspect-video mb-4" />
    ...
    <section className="py-16 sm:py-20 bg-white">
      <div className="max-w-5xl mx-auto px-5 sm:px-8">
        ...
        <div className="mb-12 flex items-center gap-4">
          ...
          <span className="text-[10px] font-black tracking-[0.28em] uppercase text-orange-500">Latest Articles</span>
        </div>
          ...
          {posts.map((post) => {
            ...
            return (
              <article key={post.id} className="group flex flex-col">
                {/* Thumbnail */}
                <Link href={`/blog/${post.slug}`} className="block overflow-hidden bg-gray-50 aspect-video mb-5 relative">
                  {img ? (
                ...
                <div className="flex items-center gap-3 mb-3">
                  <time className="text-[11px] text-slate-400 tracking-wide font-medium">{formatDate(post.date)}</time>
                  {cats.length > 0 && (
                    <>
