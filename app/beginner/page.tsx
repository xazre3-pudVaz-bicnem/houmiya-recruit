...
const faqItems = [
  ...
  { q: '厚木市の本社まで毎日通勤が必要ですか？', a: '現場直行・直帰が基本なので、毎日本社に来る必要はありません。自宅から現場に直行し、終わったら直帰できます。本社（厚木市温水西）には週に数回、朝の集合や用品補充のために立ち寄ることがあります。' },
  { q: '設備工事は将来もなくならない仕事ですか？', a: 'はい。住宅のエアコン・給湯器・電気設備は今後も必要であり続けます。AIやロボットで代替できない「現場での手作業」が中心のため、需要がなくなる可能性は低い仕事です。神奈川県全域で安定した仕事量があります。' },
  { q: '設備工事に向いていないと感じたらどうなりますか？', a: '入社後に職種が合わないと感じた場合は、担当職種の変更について上司に相談できます。エアコン工事から給湯器交換へ、電気工事と組み合わせるなど、柔軟に担当を変えられる場合があります。まずは現場で体験してから判断してください。' },
  { q: '未経験でも採用される確率は高いですか？', a: '宝宮設備は未経験者の採用を前提にした体制を整えています。学歴・前職・経験の種類は問いません。書類選考→面接という流れで、「やる気・誠実さ・コミュニケーション」を最も重視します。まずはエントリーください。' },
  { q: '入社後に後悔しないためにどんな点を確認すればよいですか？', a: '①1日の具体的な流れ（何時に出発して何件の現場を回るか）②試用期間の扱い③研修の具体的な内容④資格取得のスケジュール⑤一緒に働く先輩の雰囲気—これらを面接で直接確認することをおすすめします。宝宮設備では面接前の見学・現場同行も相談できます。' },
]
    ...
    <>
      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          {/* FAQ */}
          {/* Atsugi-specific section */}
          <div className="mb-14">
            <h2 className="text-xl font-black text-navy tracking-tight mb-5 pb-3 border-b border-gray-200">
              厚木市で未経験から設備工事を始めるということ
            </h2>
            <div className="space-y-3 text-sm text-gray-700 leading-relaxed mb-6">
              <p>
                宝宮設備の本社は神奈川県厚木市温水西1-4-39。厚木市は神奈川県の内陸中央部に位置し、東名高速・圏央道の交差点に近く、神奈川県全域の現場へのアクセスに優れています。
              </p>
              <p>
                厚木市在住の方は通勤がしやすく、現場直行・直帰のため実質の移動時間も短く済みます。
                厚木市内にはエアコン工事・給湯器交換・電気工事の需要が安定してあり、未経験スタッフが実践経験を積みやすい環境です。
              </p>
              <p>
                厚木市在住でない方も、海老名・大和・座間・伊勢原など近隣から通勤しているスタッフが在籍しています。
                神奈川県外からの移住希望者には社宅相談にも対応しています。
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { label: '厚木市在住の方', body: '本社まで通いやすく、現場直行直帰で移動が短い。市内現場なら通勤ゼロの日も。' },
                { label: '近隣市から通勤', body: '海老名・大和・座間・伊勢原などから通勤中のスタッフが在籍。本社まで車で30分圏内が目安。' },
                { label: '転居を検討中', body: '神奈川県外からの応募も歓迎。社宅相談（厚木市および近隣エリア）に対応しています。' },
              ].map((item) => (
          ...
          <div className="mb-12">
            <h2 className="text-xl font-black text-navy tracking-tight mb-5 pb-3 border-b border-gray-200">未経験の方からよくある質問（20問）</h2>
            <div className="space-y-4">
              {faqItems.map((item, i) => (
                <div key={i} className="border border-gray-200">
                  <div className="bg-gray-50 px-5 py-3.5 border-b border-gray-200">
